...
public class CancelBookingHandler : IRequestHandler<CancelBookingCommand, Result<int>>
{
    private readonly IUnitOfWork _uow;
    private readonly IBookingRepository _repo;

    public CancelBookingHandler(IUnitOfWork uow, IBookingRepository repo)
    {
        _uow = uow;
        _repo = repo;
    }

    public async Task<Result<int>> Handle(CancelBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure<int>(Domain.Common.Error.Problem("Booking.CancelFailed", ex.Message));
        }
    }
}
